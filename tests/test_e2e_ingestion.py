...
def test_e2e_ingestion_empty_query_fallback():
    ...
    Ensures empty query is handled safely through Stages 1-4.
    """
    with patch(
        "agents.document_processing.download_patent_pdf", return_value=None
    ):
...
def test_e2e_ingestion_minimal_query_handling():
    """
    End-to-End Integration Test: Minimal / Whitespace Query Handling.
    Ensures whitespace-only query is handled safely without throwing exceptions.
    """
    with patch(
        "agents.document_processing.download_patent_pdf", return_value=None
    ):
